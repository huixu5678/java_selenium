...
import com.huixu.tools.DateFormate;
...
public class TestReport extends TestListenerAdapter{
	...
	public void onStart(ITestContext context) {
		...
		File htmlReportDir = new File(path+"/testReports/"+TestReport.formateDate1());
        if (!htmlReportDir.exists()) {
        	htmlReportDir.mkdirs();
        }
        String reportName = TestReport.formateDate2()+"_result.html";
	...
	public static String formateDate1(){
		SimpleDateFormat sf = new SimpleDateFormat(DateFormate.SDF_DAY);
		Calendar cal = Calendar.getInstance();
		Date date = cal.getTime();
		return sf.format(date);
	}
	public static String formateDate2(){
		SimpleDateFormat sf = new SimpleDateFormat(DateFormate.SDF_DAY_TIMES);
		Calendar cal = Calendar.getInstance();
		Date date = cal.getTime();
		return sf.format(date);
	}
